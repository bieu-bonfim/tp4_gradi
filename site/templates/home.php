...
<?php snippet('header') ?>
  ...
  */
  ?>
  <?php if ($campaignsPage = page('campaigns')): ?>
  <ul class="home-grid">
    <?php foreach ($campaignsPage->children()->listed() as $campaign): ?>
    <li>
      <a href="<?= $campaign->url() ?>">
        <figure>
          <?php
          /*
            The `cover()` method defined in the `campaign.php`
          ...
          */
          ?>
          <?php if ($cover = $campaign->image()): ?>
          <img src="<?= $cover->resize(1024, 1024)->url() ?>" alt="<?= $cover->alt()->esc() ?>">
          <?php endif ?>
          <figcaption>
            <span>
              <span class="example-name"><?= $campaign->title()->esc() ?></span>
            </span>
          </figcaption>
        </figure>
      </a>
